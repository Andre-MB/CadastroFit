...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    ...
    // Validações adicionais, como conferir se as senhas coincidem, podem ser adicionadas

    // Consulta o banco de dados para obter a senha atual do administrador
    $query = $conn->prepare("SELECT senha FROM usuarios WHERE id = :id");
    $query->bindParam(':id', $usuario_id);
    $query->execute();
    $administrador = $query->fetch(PDO::FETCH_ASSOC);

    if ($administrador && password_verify($senha_atual, $administrador['senha'])) {
        // Senha atual correta, agora podemos atualizar a senha
...
<body>
    ...
    <div class="container">
        ...
        <div class="login-container">
            <h1>Trocar Senha</h1>
            <p>Você está trocando a senha do usuário: <?php echo ucwords($administrador['nome']); ?></p>
